...
from tsp import gibb_sampl_fast_sampling, online_gibbs_sampling, gibb_sampl_under_parametrized_sampling
...
def test_under_parametrized_sampling_log_likelihood():
    rng = np.random.default_rng(0)
    n, d = 200, 10
    lbd = 0.0005
...
def test_under_parametrized_sampling_full_observation_log_likelihood():
    rng = np.random.default_rng(0)
    n, d = 200, 10
    lbd = 0.0005
    ...
    log_liks = []
    grad_norms = []
    for mean_it, cov_it in zip(res['list_mean_full'], res['list_cov_full']):
        S_j = jnp.array(cov_it)
        mu_j = jnp.array(mean_it)
    ...
    print("log-likelihood per iterate (full observation mean/cov):")
    for it, ll in enumerate(log_liks):
        print(f"  it {it}: {ll}")

    print("gradient magnitude per iterate (full observation mean/cov):")
    for it, g in enumerate(grad_norms):
        print(f"  it {it}: {g}")
    ...
    print("avg grad norm, first third:", avg_first)
    print("avg grad norm, last third:", avg_last)
    print("gradient magnitude decreased:", avg_last < avg_first)


if __name__ == "__main__":
    # test_gradient_norm_decreases_along_gibbs_iterates()
    test_online_gibbs_decreasing_gamma_log_likelihood()
    test_under_parametrized_sampling_log_likelihood()
    test_under_parametrized_sampling_full_observation_log_likelihood()
